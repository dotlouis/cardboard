angular.module('cardboard.factories').factory('TrendsFactory', [
  '$http',
  'DefaultSettings',
  function($http, DefaultSettings) {
    ...
    function fromNetwork() {
      ...
      $http
        .get(DefaultSettings.trends.url)
        ...
        .then(
          function(res) {
            if (res.status !== 200) {
              ...
              for (var i = 0; i < 12; i++)
                if (
                  res.data[i] // some data is undefined
                )
    ...
    Trends.get = function(enabled = false) {
      return chrome.storage.local.getAsync('trends').then(function(cache) {
        // if there is nothing in cache we load trends from network
        if (!cache.trends) return fromNetwork();
        else if (
          enabled &&
          !new Date(cache.trends.lastRefresh).isSameDateAs(new Date())
        )
