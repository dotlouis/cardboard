angular.module('cardboard.factories').factory('DoodlesFactory', [
  '$http',
  'DefaultSettings',
  function($http, DefaultSettings) {
    ...
    function fromNetwork() {
      ...
      $http.get(DefaultSettings.doodles.url).then(
        function(res) {
          if (res.status !== 200) {
            ...
            const doodles = {
              url: 'https://www.google.com/doodles/' + res.data.doodles[0].name,
              img: 'https:' + res.data.doodles[0].hires_url,
            };
    ...
    Doodles.get = function(enabled = false) {
      return chrome.storage.local.getAsync('doodles').then(function(cache) {
        // if there is nothing in cache we load doodle from network
        if (!cache.doodles) return fromNetwork();
        else if (
          enabled &&
          !new Date(cache.doodles.lastRefresh).isSameDateAs(new Date())
        )
