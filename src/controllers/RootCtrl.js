  ...
  function(
    ...
    $scope.settings.then(function(settings) {
      ...
      for (var i in settings.sync.backgrounds)
        if (
          settings.sync.backgrounds[i].type == 'Local' &&
          settings.sync.backgrounds[i].url
        )
          settings.sync.backgrounds[i].url =
            settings.local.localBackgroundDataUrl;
    ...
    $scope.setBackground = function(background) {
      ...
      else $scope.backgroundImageUrl = background.url;
    };
    ...
    function getBackgroundTime(url) {
      ...
      if (hour > 5 && hour < 8) time = url.dawn;
      else if (hour > 8 && hour < 19) time = url.day;
      else if (hour > 19 && hour < 21) time = url.dusk;
      else time = url.night;
      return time;
    }
    ...
    $scope.getFavicon = function(url) {
      const regex = /(chrome:\/\/|view-source:)/gi;
      if (!regex.test(url)) return DefaultSettings.faviconURL + encodeURI(url);
      return '';
    };
