angular.module('cardboard.controllers').controller('SessionsCtrl', [
  '$scope',
  function($scope) {
    $scope.maxDevices = 5;
    ...
    var willGetRecentlyClosed = chrome.sessions
      .getRecentlyClosedAsync({
    ...
    var willGetDevices = chrome.sessions
      .getDevicesAsync({
    ...
    Promise.all([willGetDevices, willGetRecentlyClosed]).then(function() {
      $scope.$apply();
      $scope.initDropdowns('.card.sessions .dropdown-card-btn i');
