angular.module('cardboard.controllers').controller('AppCtrl', [
  '$scope',
  function($scope) {
    $scope.apps = [];
    $scope.extensions = [];
    ...
    if (chrome.management.getAll)
      chrome.management.getAll(function(all) {
        angular.forEach(all, function(value, key) {
          if (value.type == 'extension') $scope.extensions.push(value);
          else if (value.type == 'theme') $scope.themes.push(value);
          else if (value.enabled) {
            if (
              $scope.$parent.card.settings &&
              $scope.$parent.card.settings[value.id]
            )
              value.frequency =
                $scope.$parent.card.settings[value.id].frequency;
            else value.frequency = 0;
    ...
    $scope.initDropdowns('.card.apps .dropdown-card-btn i');
    ...
    $scope.getIcon = function() {
      ...
      if (!this.app.enabled) icon_url += '?grayscale=true';
      return icon_url;
    };

    $scope.updateFrequency = function() {
      if (!$scope.$parent.card.settings) $scope.$parent.card.settings = {};
      ...
      if (!$scope.$parent.card.settings[this.app.id])
        ...
        $scope.$parent.card.settings[this.app.id] = {
          frequency: $scope.$parent.card.settings[this.app.id].frequency + 1,
        };
