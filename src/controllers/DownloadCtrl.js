angular.module('cardboard.controllers').controller('DownloadCtrl', [
  '$scope',
  'fromNowFilter',
  'humanizeFilter',
  function($scope, fromNow, humanize) {
    ...
    chrome.downloads
      ...
      .then(function(downloads) {
        ...
        $scope.$apply();
        // init all dropdowns
        $scope.initDropdowns('.card.downloads .dropdown-card-btn i');
      ...
      for (i in $scope.downloads)
        if ($scope.downloads[i].id == downloadDelta.id) {
          for (j in downloadDelta)
            if (j != 'id') $scope.downloads[i][j] = downloadDelta[j].current;
          break;
        }
    ...
    chrome.downloads.onCreated.addListener(function(download) {
      ...
      Promise.delay(500)
        .then(function() {
          ...
          else throw new Error('No filename');
        })
        .then(function(dataUrl) {
    ...
    $scope.open = function() {
      if (this.download.state == 'interrupted')
        ...
        if (this.download.exists) chrome.downloads.open(this.download.id);
        else Materialize.toast('File moved or deleted', 4000);
      }
    };
    ...
    $scope.remove = function() {
      chrome.downloads
        .eraseAsync({ id: this.download.id })
    ...
    $scope.getStatus = function() {
      ...
      if (this.download.state == 'in_progress') {
        if (this.download.paused) str = 'Paused';
        else str = fromNow(this.download.estimatedEndTime);
